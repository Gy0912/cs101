...
\begin{parts}
	...
	\begin{choices}
		\choice If \(n\le M\), then the probability that there will be a hash collision is about \(\frac{n}{M}\).%\frac{n+2}{M}%
		\CorrectChoice If collisions are resolved by chaining and the load factor is \(\lambda=0.9\), the average time complexity of a successful search (accessing an element which exists in the hash table) is \(\Theta(1)\).%\Theta(n)%
		...
		\choice If collisions are resolved by quadratic probing, when the hash table is fully filled (\(n=M\)), you can reallocate a new space of size \(2M\) and copy the \(M\) bins of the old space to the first \(M\) bins of the new space (like \lstinline{std::vector}) in order to hold more elements.%need a new way to allocate%
	\end{choices}

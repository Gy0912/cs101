...
\begin{parts}
    ...
    \begin{solution} \\
        ...
        %\vspace{1.5in}
        Our certificate and certifier for $\mathsf{Set\text{-}Cover}$ goes as follows:
        \begin{enumerate}
            \item A subset $T\subseteq [n]$ and its complementary set, whose size is less than n, thus is polynomial.
            \item Check whether $\sum_{i\in T} b_i =  \sum_{j\in [n] \setminus T} b_j$, whose run time is polynomial.
        \end{enumerate}
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \end{solution}
    ...
    \begin{subparts}
        ...
        \begin{solution} \\
            ...
            %\vspace{4.5in}
            Define $X = \sum_{i\in [m]} a_i$ and let $n = m +1$. Then we define our reduction as: \\
            $$\langle{b_1, \ldots, b_{n}\rangle} = f(\langle{a_1, a_2, \ldots, a_m, k\rangle}) \overset{\Delta}{=} \langle{a_1, a_2, \ldots, a_m, 2k-X\rangle}$$
            In this way, we may deduce that $\langle{a_1, a_2, \ldots, a_m, k\rangle}$ is a $yes$-instance of $\mathsf{Subset \text{-} Sum}$ if and only if $\langle{b_1, \ldots, b_{n}\rangle} = \langle{a_1, a_2, \ldots, a_m, 2k - X\rangle}$ is a $yes$-instance of $\mathsf{Equivalent}$-$\mathsf{Partition}$ because a subset has sum $k$ and the remaining subset also has sum $k$, resulting in an equivalent partition.
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \end{solution}
    \end{subparts}

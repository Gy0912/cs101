...
\begin{parts}
    ...
    \begin{choices}
        ...
        \CorrectChoice The segmented least squares algorithm can be optimized to $\Theta(n^2)$ run time if we precompute the SSE $e_{ij}$ for all $i\le j$ in $\Theta(n^2)$ time.
    \end{choices}
    ...
    \begin{choices}
        ...
        \CorrectChoice If we use top-down dynamic programming (memorization) to compute $OPT(n)$, then we consume additional $O(n)$ function call stack space compared to bottom-up dynamic programming.
        ...
        \CorrectChoice After $OPT(j)$ for all $j\in[0,n]$ are computed, we can find one possible set of mutually compatible jobs that maximize the sum of weights in $O(n)$ time.
    \end{choices}

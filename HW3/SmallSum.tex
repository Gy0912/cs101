...
\begin{parts}
    ...
    \begin{solution} \\
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %\vspace{3cm}
        For the merge function, assume that we input a array of length m, we have three loops.
        In the first loop, all operations are $\theta(1)$, so that we only need to deal with the loop itself.
        The first loop end at $p1 = mid+1$ or $p2 = right$, which means the complexity of this loop is $\theta(m) + O(m)$.
        The complexity of second loop and the third loop are $\theta(m)$, while the program only runs one of them.
        The last loop has $\theta(m)$. Thus total outcome is $\theta(m)$.\\
        Then comes the inversions. Assume that the length of array is n, the split function will divide the array into $log(n)$ subsets with length 2.
        Make the complexity of these two functions T(n).
        \begin{equation}
            T(n) = 2 \cdot T(\frac{n}{2}) + f(n)
        \end{equation}
        Thus the whole time complexity is $\theta(nlog(n))$.
    \end{solution}
\end{parts}